...
class Order_Class
{
	 ...
	 * @return int 订单状态值 0:未知; 1:未付款等待发货(货到付款); 2:等待付款(线上支付); 3:已发货(已付款); 4:已付款等待发货; 5:已取消; 6:已完成(已付款,已收货); 7:全部退款; 8:部分发货(货到付款+已经付款); 9:部分退款(未发货+部分发货); 10:部分退款(全部发货); 11:已发货(货到付款); 12:未处理的退款申请 13:待自提 14:待成团
	 */
	...
	public static function goodsSendStatus($orderGoodsRow)
	{
	    ...
	    if(isset($orderGoodsRow['type']))
	    {
	        $isDelivery = $orderGoodsRow['type'] == 'default' ? true : false;
	    }
	    else if($orderGoodsRow['delivery_id'])
	    {
	        $isDelivery = true;
	    }
	    else if(stripos($orderGoodsRow['goods_array'],'~'))
	    {
	        $isDelivery = false;
	    }
	    else if($orderGoodsRow['goods_weight'])
	    {
	        $isDelivery = true;
	    }
	    $data = $isDelivery ? ['未发货','已发货','已退货'] : ['待付款','已付款','已退款'];
