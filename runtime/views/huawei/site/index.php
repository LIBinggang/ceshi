...
<head>
	...
	<script type="text/javascript" charset="UTF-8" src="/iWebShop5.7.0303/runtime/_systemjs/form/form.js"></script>
	<script type="text/javascript" charset="UTF-8" src="/iWebShop5.7.0303/runtime/_systemjs/autovalidate/validate.js?v=20190606"></script><link rel="stylesheet" type="text/css" href="/iWebShop5.7.0303/runtime/_systemjs/autovalidate/style.css" />
					...
					<div class="body_toolbar_layer">
						...
						<div class="myorder">
							<dl class="clearfix">
								...
								<dd><a href="<?php echo IUrl::creatUrl("/ucenter/account_log");?>">账户预付款</a></dd>
			...
			<div class="category_list">
				...
				<ul class="cat_list none">
					...
					<li>
						...
						<div class="cat_more">
							<h3>
							    <?php foreach($items=Api::run('getCategoryByParentid',array('#parent_id#',$first['id']), 7) as $key => $second){?>
								<a href="<?php echo IUrl::creatUrl("/site/pro_list/cat/".$second['id']."");?>">
									<span><?php echo isset($second['name'])?$second['name']:"";?></span>
								</a>
								<?php }?>
							</h3>

							<?php if(isset($second) && $second){?>
							<ul class="cat_nav_list">
								<?php foreach($items=Api::run('getCategoryByParentid',array('#parent_id#',$second['id']), 12) as $key => $third){?>
								<li><a href="<?php echo IUrl::creatUrl("/site/pro_list/cat/".$third['id']."");?>"><?php echo isset($third['name'])?$third['name']:"";?></a></li>
								<?php }?>
							</ul>
							<?php }?>
...
<div class="home_feature">
	...
	<div class="feature_content">
		...
		<div class="feature_title">
			...
			<a href="<?php echo IUrl::creatUrl("/site/article_detail/id/".$item['id']."");?>"><?php echo Article::showTitle($item['title'],$item['color'],$item['style']);?></a>
			<?php }?>
		</div>
	</div>
</div>
	...
	<div id="home_panic" class="act_content">
		<ul>
			...
			<li>
				...
				<!-- 限时抢购-倒计时 -->
				<a title="<?php echo isset($item['name'])?$item['name']:"";?>" href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>">
					<img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/170/h/170");?>">
					<h3 class="pro_title"><?php echo isset($item['name'])?$item['name']:"";?></h3>
...
<!--拼团活动-->
<?php $assembleList=Api::run('getAssembleList', 4);?>
<?php if($assembleList){?>
<section class="home_floor assemble">
	<header class="home_layer_title">
		<h3>拼团商品</h3>
		<a class="more" href="<?php echo IUrl::creatUrl("/site/assemble");?>">更多></a>
	</header>
	<section class="floor_body">
		<div class="floor_goods">
			<ul class="costpoint">
				<?php foreach($items=$assembleList as $key => $item){?>
				<li>
					<a class="home_pul_box_shadow" href="<?php echo IUrl::creatUrl("/site/assemble/id/".$item['id']."");?>">
						<img class="floor_goods_img" title="<?php echo isset($item['title'])?$item['title']:"";?>" src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/170/h/170");?>">
						<h3 class="pro_title"><?php echo isset($item['title'])?$item['title']:"";?></h3>
						<del>￥<?php echo isset($item['sell_price'])?$item['sell_price']:"";?></del>
						<p class="pro_price">￥<?php echo isset($item['assemble_price'])?$item['assemble_price']:"";?></p>
					</a>
				</li>
				<?php }?>
			</ul>
	...
	<section class="floor_body">
		<div class="floor_goods">
			<ul class="costpoint">
				...
				<li>
					<a class="home_pul_box_shadow" href="<?php echo IUrl::creatUrl("/site/products/id/".$item['goods_id']."");?>">
...
<!--积分兑换-->
<!-- 开始首页分类 -->
<?php foreach($items=Api::run('getCategoryListTopByVis') as $key => $first){?>
<section class="home_floor">
	<header class="home_layer_title">
		<h3><?php echo isset($first['name'])?$first['name']:"";?></h3>
		<nav class="floor_nav">
			<ul>
				<?php foreach($items=Api::run('getCategoryByParentidByVis',array('#parent_id#',$first['id'])) as $key => $second){?>
